...
export async function sendSms(to: string, text: string): Promise<void> {
  ...
  try {
    const res = await axios.post(
      url,
      { key: CALLPRO_KEY, from: CALLPRO_FROM, to: String(to).trim(), text: String(text) },
      {
        headers: {
    ...
    if (err.response) {
      ...
      throw new Error(`CallPro SMS API failed: Status ${err.response.status} - ${JSON.stringify(err.response.data)}`);
    }
    throw err;
  }
}
